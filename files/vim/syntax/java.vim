...
syn region  javaComment		 start="/\*"  end="\*/" contains=@javaCommentSpecial,javaTodo,@Spell,javaDoc extend keepend
...
syntax match   javaObjectProp     /\.\<\K\k*/ms=s+1  skipwhite skipempty containedin=javaFold,javaParenT,javaParenT1

syntax match javaFuncName /\<\K\k*(/me=e-1 skipwhite skipempty containedin=javaFold,javaParenT,javaParenT1


hi def link javaFuncName Function
hi def link javaObjectProp jsObjectProp


syn region javaDoc start=/\/\*\*/ end=/\*\// contained transparent contains=javaDocReturnLine,javaDocParamLine,javaDocNormalLine,javaDocSpec,javaSpecMultiLine

syn region javaDocNormalLine start="@[^\s]\+" end="$" transparent contained contains=javaDocTag,javaDocSpec,javaSpecMultiLine
syn region javaDocReturnLine start="@return"  end="$" transparent contained contains=javaDocReturnTag,javaDocSpec,javaSpecMultiLine
syn region javaDocParamLine  start="@param"   end="$" transparent contained contains=javaDocParamTag,javaDocSpec,javaSpecMultiLine

syn match javaDocTag  "@\<\(abstract\|access\|author\|category\|copyright\|deprecated\|example\|final\|global\|ignore\|internal\|license\|link\|method\|name\|package\|param\|property\|return\|see\|since\|static\|staticvar\|subpackage\|tutorial\|uses\|var\|version\|contributor\|modified\|filename\|description\|filesource\|throws\)\(\s\+\)\?\>" contained

syn match javaDocParamTag  "@param" contained nextgroup=javaDocParamVarName skipwhite
syn match javaDocParamVarName '\w\+' contained

syn match javaDocReturnTag  "@returns\?" contained nextgroup=javaDocReturnType skipwhite
syn match javaDocReturnType "{\<[a-zA-Z]\+\>}" contained

syn region javaSpecMultiLine start="---" end="---" contained contains=basicComment,javaDocSpecShow
syn match  javaDocSpec      "--.\{-}--" contained contains=javaDocSpecShow
syn match  javaDocSpecShow  "<[^>]\+>" contained
syn match  basicComment   "^\s*\*" contained

syn match typesSeparator "|" containedin=javaDocReturnType


hi def link javaDocType     commDocType
hi def link javaDocVarName  commDocVarName
hi def link javaDocTag      commDocTag
hi def link javaDocSpec     commDocSpec
hi def link javaDocSpecShow commDocSpecShow

hi def link javaDocReturnType javaDocType
hi def link javaDocParamTag   javaDocTag
hi def link javaDocReturnTag  javaDocTag
hi def link javaDocParamVarName javaDocVarName

hi def link javaSpecMultiLine javaDocSpec
