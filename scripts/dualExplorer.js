...
export default class DualExplorer extends Explorer {
    ...
    open(recursive = false) {
        if (!this.currentFile.isDirectory) {
            ...
            if (this.currentFile2.isDirectory)
                this.currentFile2.open();
            this.refreshDisplay();
        }
    }
    ...
    goUp(toFirst) {
        if (toFirst) {
            ...
            if (this.currentFile2.parent) {
                const kid2 = this.currentFile2.parent.children.find(kid => kid.name === this.currentFile.name);
                if (kid2)
                    this.currentFile2 = kid2;
            }
        }
        else {
...
//# sourceMappingURL=dualExplorer.js.map