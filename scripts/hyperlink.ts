...
async function logLink(link: string) {
  const forbiddenPaths = ["/node_modules/", "/.git/", "/__cache__/"];

  if (forbiddenPaths.some((path) => link.includes(path))) {
    return;
  }

  fs.appendFileSync("/tmp/hyperlogs.log", removeColors(link) + "\n");
}
...
async function getLinks() {
  if (!fs.existsSync("/tmp/hyperlogs.log")) {
    console.log("No links found");
    process.exit(0);
  }

  const links = fs
    .readFileSync("/tmp/hyperlogs.log", "utf-8")
    .split("\n")
    .map((line) => removeColors(line))
    .filter((link) => {
      let linkPart = link.split(":")[0];
      if (!linkPart) {
        return false;
      }

      if (!fs.existsSync(linkPart.trim())) return false;

      const cwd = process.cwd();
      // We make sure that the file is under cwd
      const fullPath = fs.realpathSync(linkPart.trim());

      if (!fullPath.startsWith(cwd)) {
        return false;
      }

      return fs.existsSync(linkPart.trim());
    });
