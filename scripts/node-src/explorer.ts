...
import { Cli, CliColor, StyleOptions } from './cli.js';
...
class File {
    ...
    opened = false;
    maxDepth: number;
    ignorePatterns: RegExp[] = [/.git\//, /node_modules\//];

    constructor(path: string, maxDepth = -1, ignorePatterns = []) {
    ...
    private findKidsNames(): string[] {
        ...
        kidsNames = kids
            .filter(kid => {
                if (!kid.isFile() && !kid.isDirectory()) return false;
    ...
    getChildAbsolutePos(child: File) {
        if (!this.isDirectory || !this.opened || !this.children.length)
            return -1;
...
class Explorer {
    ...
    fileOpenListener: (file: File) => unknown = null;

    constructor(path: string, onFileOpen: (file: File) => unknown) {
...
async function explore(path: string): Promise<File> {
