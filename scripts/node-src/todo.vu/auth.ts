...
function interceptCookies(res: Response): Response {
...
function parseCookie(raw: string): [string, string] {
    return raw.replace(/;.*$/, '').split(/=/) as [string, string];
}

export function headers(
    opts: {
        with_csrf_token?: boolean;
    } = {},
...
function loadToken(): void {
    if (!fs.existsSync('/tmp/.todo.token')) return;
    CSRF_TOKEN = fs.readFileSync('/tmp/.todo.token').toString();
}
function loadCookies(): void {
    if (!fs.existsSync('/tmp/.todo.cookies')) return;
...
export async function login(
    ...
    if (!CSRF_TOKEN || !Object.keys(COOKIES).length) {
        ...
        await fetch('https://todo.vu/login', {
            method: 'post',
            body: form.toString(),
            redirect: 'manual',
            headers: headers(
