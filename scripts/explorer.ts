...
class ExplorerFile extends File {
  ...
  async openToPath(path: string): Promise<ExplorerFile> {
    if (!this.isDirectory || path === this.path || !path.startsWith(this.path))
      return this;
  ...
  async isVisible(): Promise<boolean> {
    ...
    if (this.gitOnly) {
      ...
      if (gitState === GitFileState.NONE || gitState === GitFileState.IGNORED)
        return false;
    }
  ...
  async getClosestVisible(): Promise<ExplorerFile> {
    ...
    while (!(await f.isVisible())) {
      f = (f.previousSibling as ExplorerFile) ?? (f.parent as ExplorerFile);
    }
    return f;
  }
  ...
  async getChildAbsolutePos(child: ExplorerFile): Promise<number> {
    if (!this.isDirectory || !this.opened || !this.children.length) return -1;
...
class Explorer {
  ...
  constructor(path: string, onFileOpen?: (file: ExplorerFile) => unknown) {
    ...
    this.cli.waitForReady.then(() => {
      this.cli.maxHeight = this.cli._termMetas.height;
  ...
  async createNew(isDir = false): Promise<void> {
    ...
    this.cli.y = (await this.currentFile.getPosition()) - this.offset;
    this.refreshDisplay();
  ...
  async goToNextFile(): Promise<void> {
    if (this.currentFile.isDirectory && !this.currentFile.opened) {
    ...
    let cnt = 0;
    while (this.currentFile.isDirectory) {
      if (this.currentFile.isDirectory && !this.currentFile.opened) {
        this.currentFile.open();
      }
      this.currentFile = await this.currentFile.getNext();
  ...
  async goToPrevFile(): Promise<void> {
    if (this.currentFile === this.rootFile) return;
    ...
    while (this.currentFile.isDirectory) {
      if (goingUp) {
        const siblings = await this.currentFile.parent!.getVisibleChildren();
        const idx = siblings.indexOf(this.currentFile);
        if (idx === 0) this.currentFile = this.currentFile.parent!;
        else {
          goingUp = false;
          this.currentFile = siblings[idx - 1];
        }
      } else {
        if (!this.currentFile.opened) this.currentFile.open();
        const kids = await this.currentFile.getVisibleChildren();
        if (kids.length) this.currentFile = kids[kids.length - 1];
        else {
          goingUp = true;
      ...
      if (this.currentFile == null) this.currentFile = this.rootFile;
      if (this.currentFile == this.rootFile) break;
  ...
  async onInput(
    ...
    if (keyName.length === 1) {
      switch (keyName) {
        ...
        case "k":
          if (shift) await this.goToPrevFile();
          else await this.goUp(false);
          break;

        case "j":
          if (shift) await this.goToNextFile();
          else await this.goDown(false);
          break;
      ...
      switch (keyName) {
        ...
        case "backspace":
          if (this.currentFile.parent) {
            ...
            try {
              this.rootFile = this.rootFile.findParent() as ExplorerFile;
              this.currentFile = this.rootFile;
              this.rootFile.open();
            } catch (err) {
...
if (/explorer\.[jt]s$/.test(process.argv[1])) {
  let inPath = "";
