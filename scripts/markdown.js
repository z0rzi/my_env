...
import { findMistakes } from './correct.js';
...
function onFileSave() {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const lineNr = splittedMd.findIndex((lineContent, lineIdx) => lineContent !== lastMdContent[lineIdx]);
        lastMdContent = splittedMd;
        if (lineNr > 0)
            currentMdLine = lineNr;
        if (currentMdLine > 0) {
            const lineSave = splittedMd[currentMdLine];
            let line = splittedMd[currentMdLine];
            const errors = yield findMistakes(line);
            errors.reverse().forEach(mistake => {
                let mistakeId = 0;
                while (mistakeId in mistakes)
                    mistakeId++;
        ...
        html = html.replace(/@\d+::.*?::.*?@/g, error => {
            const { groups } = error.match(/^@(?<id>\d+)::(?<mistake>.*?)::(?<fix>.*?)@$/);
        ...
        const currentLine = splittedHtml.findIndex(line => line.includes('@cursor@'));
        if (currentLine >= 0) {
        ...
        serv = http
            ...
            if (req.url === '/should-reload') {
                ...
                if (!fs.existsSync(assetPath)) {
                    assetPath = path.join(path.dirname(markdownFile), assetPath);
                }
                if (!fs.existsSync(assetPath)) {
                    return;
                }
