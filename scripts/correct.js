...
const body = [
    ...
    'texteHTML=' +
        encodeURIComponent(`[[[PARTTEXT]]]p0;p26;[[[TEXT]]]<p>${content}</p>`),
    'texteStat=',
    'IdMax=56',
    ...
    'charPrecPh=-1',
    'optionsCor=' +
        encodeURIComponent('Genre_Je:0|Genre_Tu:0|Genre_Nous:0|Genre_Vous:0|Genre_On:0|RefOrth:0|ShowUPSol:1'),
    'optionsStyle=' +
        encodeURIComponent('RepMin:3|GapRep:3|AllWords:0|FamilyWords:0|MinPhLg:30|MinPhCt:5|Ttr:250|Tts:150'),
    'ensIdRepetitions=',
...
fetch('https://www.scribens.fr/Scribens/TextSolution_Servlet', {
    ...
    .then(res => {
    const html = res.VectPartText.map(e => e.TexteHtML).join('');
    const node = new HtmlNode(`<div>${html}</div>`);
    node.decodeSpecial();
    const replacements = [];
    Object.values(res.SolutionCor.MapMotSolution).forEach((repl) => {
        replacements.push({
            id: repl.Id,
