...
let MAX_ARR_SIZE = 20;
if (process.argv.indexOf('--full') >= 0) MAX_ARR_SIZE = 10000;
...
function getType(obj) {
    ...
    if (typeof obj === 'string') {
        if (!isNaN(new Date(obj).getTime())) return 'Date';
        try {
            JSON.parse(obj);
            return 'json';
        } catch (err) {}
...
function toTypescript(obj, idt = 0) {
        ...
        if (obj.every(elem => getType(elem) === firstType)) {
            ...
            obj.forEach(child => {
                keys.filter(k => k.sure).forEach(k => {
                    if (!(k.key in child)) k.sure = false;
                });
            ...
            return `{${newline(idt + 1) + str + newline(idt)}}[] /* x${
                obj.length
            } */`;
        } else {
            ...
            obj.forEach(elem => {
                ...
                if (!childs.includes(str)) childs.push(str);
            });
            return `[(${
                newline(idt + 1) + childs.join(' | ') + newline(idt)
            })]`;
        }
    }
    if (objType === 'object') {
    ...
    if (objType === 'json') {
        ...
        return `/* START_JSON_STR { */${
            newline(idt + 1) + toTypescript(js, idt + 1) + newline(idt)
        }/* } END_JSON_STR */`;
    }
