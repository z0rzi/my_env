...
class Cli {
  ...
  async refreshTermMetas(): Promise<void> {
    this._termMetas.height = await $`tput lines`.text().then(Number);
    this._termMetas.width = await $`tput cols`.text().then(Number);
  ...
  updateHeight(newHeight: number): void {
    ...
    if (availableSpace < this.maxHeight) {
      ...
      this._termMetas.offset.lines = this._termMetas.height - this.maxHeight;
    }
  }
  ...
  write(text: string, styleOpts: StyleOptions = {}): void {
    ...
    if (styleOpts && "italic" in styleOpts && styleOpts.italic) this.italic();
